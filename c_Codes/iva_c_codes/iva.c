...
iva_parameters* newparameters(int nfft, int framelen, int frame_shift, int maxiter)
{
   iva_parameters* newparameters = (iva_parameters*)malloc(sizeof(iva_parameters));
    ...
    newparameters->x1 = (double**)calloc(160000+(2*nfft), sizeof(double));
    ...
    newparameters->yf1=(double*)calloc(160800,sizeof(double));
    newparameters->yf2=(double*)calloc(160800,sizeof(double));
    
    newparameters->xi_w1=(double*)calloc(nfft,sizeof(double));
    newparameters->xi_w2=(double*)calloc(nfft,sizeof(double));
    
    newparameters->win2=(double*)calloc(nfft,sizeof(double));
    newparameters->x_prev1=(double*)calloc(nfft,sizeof(double));
    newparameters->x_prev2=(double*)calloc(nfft,sizeof(double));
    newparameters->v1prev=(double*)calloc(nfft,sizeof(double));
    newparameters->x1_prev=(double*)calloc(nfft,sizeof(double));
    newparameters->x2_prev=(double*)calloc(nfft,sizeof(double));
...
void enhance_signal(iva_parameters *iva, double *input1, double *input2)
{
    ...
    iva->overlap = 0.5*iva->nfft;
    iva->shift = iva->nfft - iva->overlap;
    iva->nframes = ((160000 + iva->nfft)/iva->shift) - 1;
    iva->nfreq = (iva->nfft/2)+1;
    for (int i = iva->nfft; i < iva->nfft + 160000; i++)
    {
        iva->x1[i-iva->nfft][0] = input1[i-iva->nfft];
        iva->x1[i-iva->nfft][1]= input2[i-iva->nfft];
        //printf("%d %f\n",i, sub_data1[i]);
    }
    int kk=0;
    for(iva->n=1;iva->n<iva->nframes;iva->n++)
    {
         //iva->n++;
        int kk1=kk;
       // printf("%d\n",iva->nframes);
        iva->sp = ((iva->n-1)*iva->shift)+1;
        //printf("%d\n",iva->sp);
        if(iva->n<101)
        {
            for(int i=0;i<iva->nfft/2;i++)
            {
                iva->xwin[i][0]=iva->win[i]*iva->x1_prev[i];
                iva->x1_prev[i]=iva->x1[iva->sp+i-1][0];
                iva->xwin[i+iva->nfft/2][0]=iva->win[i+iva->nfft/2]*iva->x1[iva->sp+i-1][0];

                iva->xwin[i][1]=iva->win[i]*iva->x2_prev[i];
                iva->x2_prev[i]=iva->x1[iva->sp+i-1][1];
                iva->xwin[i+iva->nfft/2][1]=iva->win[i+iva->nfft/2]*iva->x1[iva->sp+i-1][1];
//                iva->xwin[i][0]=iva->win[i]*iva->x1[kk1][0];
//                iva->xwin[i][1]=iva->win[i]*iva->x1[kk1][1];
//                kk1++;
            }
            ...
            for(int i=0;i<iva->nfft/2+1;i++)
            {
                ...
                for(int i=0;i<iva->nfft/2+1;i++)
                {
                    iva->Xn_1_real[i][iva->n-1]=iva->xn1_real[i];
                    iva->Xn_2_real[i][iva->n-1]=iva->xn2_real[i];
                    iva->Xn_1_img[i][iva->n-1]=iva->xn1_imag[i];
                    iva->Xn_2_img[i][iva->n-1]=iva->xn2_imag[i];
               //printf("%.32lf %d\n",(iva->Xn_2_img[i][iva->n-1]),iva->n-1);
                }
            kk=kk+iva->nfft/2;
        }
//        else
...
//        iva->Q2[iva->nfreq-1]=-iva->Q2[iva->nfreq-1];
    ...
    if(iva->n>101&&iva->n<=iva->maxiter)
    {
        iva->dlw=0;
        for(int k =0; k<iva->nfreq;k++)
        {
...
//printf("%.32lf\n",iva->Ssq1_2[i]);
        ...
        iva->dobj=iva->pobj-iva->obj;
        iva->pobj=iva->obj;
//printf("%.32lf\n",iva->dlw);
       // printf("%.32lf\n",iva->obj);
//        if ((fabs(iva->dobj)/fabs(iva->obj))<1e-6)
//        {
//            break;
//        }
...
//    }
    ...
    //for(int n = 1; n<=iva->nframes;n++)
    if(iva->n>iva->maxiter+1&&iva->n<iva->nframes)
    {
        for(int k =0; k<iva->nfreq;k++)
        {
            //printf("%.32lf\n",iva->fw_1_r[k]);
        ...
        {
            //printf("%.32lf\n",iva->X_final1_i[i]);
            iva->xi_w1[i]= (iva->win[i]*Y1->real[i]);
            iva->xi_w2[i]= (iva->win[i]*Y2->real[i]);
            iva->win2[i]=(iva->win[i]*iva->win[i]);
 
        }
        for(int i=0;i<iva->nfft/2;i++)
        {
            iva->sk1[i]=iva->x_prev1[i]+iva->xi_w1[i];
            iva->v1[i]=iva->v1prev[i]+iva->win2[i];
            iva->x_prev1[i]=iva->xi_w1[i+(iva->nfft/2)];
            iva->sk2[i]=iva->x_prev2[i]+iva->xi_w2[i];
            iva->v2[i]=iva->v1prev[i]+iva->win2[i];
            iva->x_prev2[i]=iva->xi_w2[i+(iva->nfft/2)];
            iva->v1prev[i]=iva->win2[i+(iva->nfft/2)];
        }
        int kkk1=k1;;
        for(int i=0;i<iva->nfft/2;i++)
        {
            iva->yf1[kkk1]=iva->sk1[i]/iva->v1[i];
            iva->yf2[kkk1]=iva->sk2[i]/iva->v2[i];
            kkk1++;
