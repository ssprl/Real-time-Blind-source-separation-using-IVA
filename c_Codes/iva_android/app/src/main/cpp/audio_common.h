...
#ifndef AUDIOTWOMICS_AUDIO_COMMON_H
#define AUDIOTWOMICS_AUDIO_COMMON_H
...
typedef struct Timer {
    unsigned int totalRuns,runFilter,runComp;
    unsigned long long totalTime,totalFiltering,totalCompute;
    struct timespec startTime;
    struct timespec stopTime;
    struct timespec computeStart,computeStop,filteringStart,filteringStop;
} Timer;

Timer* newTimer();

void startcomputeTimer(Timer* timer);
void stopcomputeTimer(Timer* timer);
void startfilteringTimer(Timer* timer);
void stopfilteringTimer(Timer* timer);
void startTimer(Timer* timer);
void stopTimer(Timer* timer);
void tellTimerTime(Timer* timer);
float getTimerMS(Timer* timer);
float getCT(Timer* timer);
float getFT(Timer* timer);
 ...
 */
//#define ENABLE_LOG  1
#endif //AUDIOTWOMICS_AUDIO_COMMON_H
